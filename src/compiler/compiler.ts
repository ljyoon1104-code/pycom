...
function compileProgram(nodes: Statement[]): Bytecode {
  ...
  const expression = (node: Expr): void => {
    switch (node.kind) {
      ...
      case "call":
        if (node.callee.kind === "attribute") { expression(node.callee.object); node.args.forEach(expression); node.keywords.forEach(keyword => expression(keyword.value)); emit({ op: "call_method", name: node.callee.name, argc: node.args.length, keywords: node.keywords.map(keyword => keyword.name), ...point(node.token) }); }
...
function localNames(nodes: Statement[], params: string[], globals: string[]): string[] { const names = new Set(params); const add = (target: Target) => { if (target.kind === "name") names.add(target.id); else if (target.kind === "unpack") target.values.forEach(add); }; const visit = (items: Statement[]) => items.forEach(node => { if (node.kind === "import") names.add(node.binding); else if (node.kind === "try") { visit(node.body); node.handlers.forEach(handler => { if (handler.name) names.add(handler.name); visit(handler.body); }); if (node.otherwise) visit(node.otherwise); } else if (node.kind === "with") { names.add(node.name); visit(node.body); } else if (node.kind === "assign") node.targets.forEach(add); else if (node.kind === "augassign") add(node.target); else if (node.kind === "for") { add(node.target); visit(node.body); } else if (node.kind === "if") { node.branches.forEach(branch => visit(branch.body)); if (node.otherwise) visit(node.otherwise); } else if (node.kind === "while") visit(node.body); }); visit(nodes); globals.forEach(name => names.delete(name)); return [...names]; }
