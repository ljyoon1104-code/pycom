...
const reserved = new Set(["import", "from", "True", "False", "None", "and", "or", "not", "is", "if", "elif", "else", "while", "for", "in", "break", "continue", "def", "return", "global", "nonlocal", "lambda", "class", "pass", "with", "as", "try", "except", "raise", "finally", "del", "assert"]);
...
export function lex(source: string): Token[] {
  ...
  for (let row = 0; row < lines.length; row++) {
    ...
    while (index < text.length) {
      ...
      const start = column;
      if (/[0-9]/.test(char) || (char === "." && /[0-9]/.test(text[index + 1] ?? ""))) { const raw = text.slice(index).match(/^(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?/)![0]; index += raw.length; column += raw.length; add("number", raw, line, start, Number(raw)); continue; }
