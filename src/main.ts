...
const chooseUnsaved = (): Promise<"save" | "discard" | "cancel"> => new Promise(resolve => { const buttons = dialog.querySelectorAll<HTMLButtonElement>("button"); const done = (choice: "save" | "discard" | "cancel") => { closeLayer(dialog); buttons.forEach(button => button.onclick = null); resolve(choice); }; buttons.forEach(button => button.onclick = () => done(button.dataset.choice as "save" | "discard" | "cancel")); openLayer(dialog, dialog.querySelector<HTMLButtonElement>('[data-choice="save"]')!, () => done("cancel")); });
