...
const invalid = (message: string, type: "ValueError" | "TypeError" | "OverflowError" | "ZeroDivisionError" = "ValueError"): never => { throw new CollectionError(type, message); };
export function mathBuiltin(name: string, args: Value[], keywords: Record<string, Value>): Value {
  ...
  if (fn === "ceil" || fn === "floor" || fn === "trunc") { if (!Number.isFinite(x)) return invalid("무한대 또는 NaN을 정수로 바꿀 수 없습니다.", Number.isNaN(x) ? "ValueError" : "OverflowError"); return fn === "ceil" ? Math.ceil(x) : fn === "floor" ? Math.floor(x) : Math.trunc(x); }
  ...
  if (fn === "log" && values.length === 2 && (values[1] <= 0 || values[1] === 1)) return invalid("로그의 밑은 양수이며 1이 아니어야 합니다.", values[1] === 1 ? "ZeroDivisionError" : "ValueError");
  ...
  if (!Number.isFinite(result) && !Number.isNaN(result) && values.every(Number.isFinite)) return invalid("계산 결과가 실수 범위를 초과했습니다.", "OverflowError"); return float(result);
}
