...
import { MemoryFileStore, checkedPut } from "../files/storage";
...
describe("6단계 파일", () => {
  ...
  it("모드·경로·닫힌 파일·권한 오류를 학생 오류로 처리한다", () => { for (const [code, line] of [['open("none.txt")', 1], ['open("../memo.txt")', 1], ['open("memo.txt", "rb")', 1], ['open("memo.txt", encoding="euc-kr")', 1], ['file=open("memo.txt", "r")\nfile.close()\nfile.read()', 3], ['file=open("memo.txt", "r")\nfile.write("x")', 2], ['file=open("memo.txt", "w")\nfile.read()', 2], ['file=open("memo.txt", "w")\nfile.write(1)', 2]] as [string, number][]) { const result = error(code, { "memo.txt": "x" }); expect(result.error.line).toBe(line); expect(result.error.message).not.toMatch(/Error|stack|undefined/); } });
  it("메모리 저장소는 명시적 저장과 제한을 처리한다", async () => { const store = new MemoryFileStore(); await checkedPut(store, { name: "main.py", content: "print(1)", updatedAt: 1 }); expect((await store.get("main.py"))?.content).toBe("print(1)"); await expect(checkedPut(store, { name: "../bad.py", content: "", updatedAt: 1 })).rejects.toThrow("invalid-name"); await expect(checkedPut(store, { name: "large.txt", content: "x".repeat(1_000_001), updatedAt: 1 })).rejects.toThrow("file-limit"); });
});
