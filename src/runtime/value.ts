...
export type Value = number | string | boolean | null | RangeValue | IteratorValue | BuiltinValue | FunctionValue | ClassValue | InstanceValue | BoundMethodValue | FileValue | ErrorTypeValue | ExceptionValue | MapValue | ListValue | TupleValue | DictValue | SliceValue | AttrValue;
...
export const keyOK = (value: Value) => !isList(value) && !isDict(value) && !isSlice(value) && !isIterator(value) && !isMap(value) && !isBuiltin(value) && !isFunction(value) && !isClass(value) && !isInstance(value) && !isBoundMethod(value) && !isFile(value) && !isErrorType(value) && !isException(value) && !isAttr(value);
export const repr = (value: Value): string => value === null ? "None" : typeof value === "number" ? String(value) : typeof value === "boolean" ? (value ? "True" : "False") : typeof value === "string" ? `'${value.replace(/'/g, "\\'")}'` : isList(value) ? `[${value.items.map(repr).join(", ")}]` : isTuple(value) ? `(${value.items.map(repr).join(", ")}${value.items.length === 1 ? "," : ""})` : isDict(value) ? `{${value.entries.map(entry => `${repr(entry.key)}: ${repr(entry.value)}`).join(", ")}}` : isRange(value) ? `range(${value.start}, ${value.stop}, ${value.step})` : isBuiltin(value) ? `<built-in function ${value.name}>` : isFunction(value) ? `<function ${value.name}>` : isException(value) ? value.message : isErrorType(value) ? `<class '${value.name}'>` : isFile(value) ? `<파일 '${value.name}'>` : isInstance(value) ? `<${value.classValue.name} 객체>` : isClass(value) ? `<class '${value.name}'>` : isMap(value) ? "<map object>" : "<값>";
export const printable = (value: Value) => typeof value === "string" ? value : repr(value);
export const typeName = (value: Value): string => value === null ? "NoneType" : typeof value === "number" ? (Number.isInteger(value) ? "int" : "float") : typeof value === "string" ? "str" : typeof value === "boolean" ? "bool" : isList(value) ? "list" : isTuple(value) ? "tuple" : isDict(value) ? "dict" : isRange(value) ? "range" : isMap(value) ? "map" : isBuiltin(value) || isFunction(value) ? "function" : "object";
