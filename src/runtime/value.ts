...
export type Value = number | string | boolean | null | ModuleValue | DateValue | TurtleValue | TurtleMethodValue | RangeValue | IteratorValue | BuiltinValue | FunctionValue | ClassValue | InstanceValue | BoundMethodValue | FileValue | ErrorTypeValue | ExceptionValue | MapValue | ListValue | TupleValue | DictValue | SliceValue | AttrValue;
...
export const keyOK = (value: Value) => !isModule(value) && !isList(value) && !isDict(value) && !isSlice(value) && !isIterator(value) && !isMap(value) && !isBuiltin(value) && !isFunction(value) && !isClass(value) && !isInstance(value) && !isBoundMethod(value) && !isFile(value) && !isTurtle(value) && !isTurtleMethod(value) && !isErrorType(value) && !isException(value) && !isAttr(value);
