...
import { exceptionMatches, pythonErrorNames } from "../compiler/python-errors";
import { elementEqual, isSuper, isErrorType, classAttribute, subclass } from "./value";
...
const ERROR_TYPES = pythonErrorNames;
...
export class VM {
  ...
  private pythonFail(i: Instruction, type: string, message: string): never { throw new VMFailure({ line: i.line, column: i.column, category: type === "UnboundLocalError" ? "name" : "value", message }, type); }
    ...
    while (true) {
      ...
      if (region && region.depth === this.frames.length) {
        ...
        if (region.kind === "except" && action.kind === "throw") {
          ...
          const match = region.handlers.find(handler => !handler.type || handler.type === "Exception" || exceptionMatches(failure.type, handler.type));
          if (match) {
            this.stack.length = region.stackHeight;
  ...
  private loadName(instruction: Instruction, name: string): Value {
    if (!this.globalNames.has(name)) {
      ...
      if (this.localNames.has(name)) return this.pythonFail(instruction, "UnboundLocalError", `지역 변수 '${name}'에 아직 값이 없습니다.`);
  ...
  private step(instruction: Instruction): void {
    ...
    if (instruction.op === "delete_subscript") {
      let key = this.pop(instruction); const object = this.pop(instruction);
      if (isList(object)) {
        if (isSlice(key)) { const slice = key, indices = this.library(instruction, () => sliceIndices(object.items.length, slice)); indices.sort((a, b) => b - a).forEach(index => object.items.splice(index, 1)); return; }
      ...
      if (isDict(object)) { this.library(instruction, () => hashable(key)); const index = object.entries.findIndex(entry => elementEqual(entry.key, key)); if (index < 0) this.fail(instruction, "value", "삭제할 키가 없습니다."); object.entries.splice(index, 1); return; }
    ...
    if (instruction.op === "binary" && ["in", "not in"].includes(instruction.operator) && isSet(this.stack.at(-1)!)) {
      const right = this.pop(instruction), left = this.pop(instruction); this.library(instruction, () => hashable(left)); const found = isSet(right) && right.items.some(item => elementEqual(item, left)); this.stack.push(instruction.operator === "in" ? found : !found); return;
    }
    ...
    if (instruction.op === "binary") { const right = this.pop(instruction), left = this.pop(instruction); if(instruction.operator==="in"||instruction.operator==="not in"){const found=typeof right==="string"&&typeof left==="string"?right.includes(left):isList(right)||isTuple(right)?right.items.some(x=>elementEqual(x,left)):isDict(right)?right.entries.some(e=>elementEqual(e.key,left)):this.fail(instruction,"type","이 값은 검사할 수 없습니다.");this.stack.push(instruction.operator==="in"?found:!found);}else this.stack.push(this.binary(instruction, left, right)); return; }
    if (instruction.op === "compare_step") {
      const right = this.pop(instruction), left = this.pop(instruction);
      if (this.compare(instruction, instruction.operator, left, right)) this.stack.push(right);
      else { this.stack.push(false); this.pc = instruction.target; }
      return;
    }
  ...
  private iterator(i: Instruction, value: Value): IteratorValue | Extract<Value, { kind: "map" }> { if (isIterator(value) || isMap(value)) return value; if (isFile(value)) { if (value.closed) return this.fail(i, "value", "닫힌 파일은 사용할 수 없습니다."); if (value.mode !== "r") return this.pythonFail(i, "UnsupportedOperation", "쓰기 모드로 연 파일에서는 읽을 수 없습니다."); return { kind: "iterator", values: [], index: 0, file: value }; } if (typeof value === "string") return { kind: "iterator", values: [...value], index: 0 }; if (isSet(value) || isDict(value)) { const size = () => isSet(value) ? value.items.length : value.entries.length, initialSize = size(); return { kind: "iterator", values: isSet(value) ? value.items : value.entries.map(entry => entry.key), index: 0, validate: () => size() === initialSize ? undefined : "반복 중에는 딕셔너리나 집합의 크기를 변경할 수 없습니다." }; } if (isList(value) || isTuple(value)) return { kind: "iterator", values: value.items, index: 0 }; if (isDict(value)) return { kind: "iterator", values: value.entries.map(entry => entry.key), index: 0 }; if (isRange(value)) return { kind: "iterator", values: this.rangeItems(i, value), index: 0 }; return this.fail(i, "type", "이 값은 반복할 수 없습니다."); }
  ...
  private subscript(i: Instruction, object: Value, key: Value): Value {
    if (isSlice(key)) {
      ...
      const slice = key, values = this.items(i, object), indices = this.library(i, () => sliceIndices(values.length, slice)), result = indices.map(index => values[index]);
      return typeof object === "string" ? result.join("") : { kind: isTuple(object) ? "tuple" : "list", items: result };
    }
    if (isDict(object)) { this.library(i, () => hashable(key)); const found = object.entries.find(entry => elementEqual(entry.key, key)); if (!found) this.fail(i, "value", `딕셔너리에 ${repr(key)} 키가 없습니다.`); return found.value; }
  ...
  private setSubscript(i:Instruction,o:Value,k:Value,v:Value):void{if(isList(o)){if(typeof k==="boolean")k=Number(k);if(typeof k!=="number"||!Number.isInteger(k))this.fail(i,"type","인덱스는 정수여야 합니다.");const x=k<0?o.items.length+k:k;if(x<0||x>=o.items.length)this.fail(i,"value",`리스트에 ${k}번 위치의 값이 없습니다.`);o.items[x]=v;return;}if(isDict(o)){if(!keyOK(k))this.fail(i,"type","변경 가능한 값은 딕셔너리 키가 될 수 없습니다.");const found=o.entries.find(e=>elementEqual(e.key,k));if(found)found.value=v;else o.entries.push({key:k,value:v});return;}this.fail(i,"type","이 값의 요소는 수정할 수 없습니다.");}
  private method(i:Extract<Instruction,{op:"call_method"}>,o:Value,n:string,a:Value[]):void{if(isFile(o)){if(n==="read"&&(a.length===0||a.length===1)){this.stack.push(this.fileRead(i,o,a[0]));return;}if(n==="readline"&&a.length===0){if(o.closed)this.fail(i,"value","닫힌 파일은 사용할 수 없습니다.");if(o.mode!=="r")this.pythonFail(i,"UnsupportedOperation","쓰기 모드로 연 파일에서는 읽을 수 없습니다.");this.stack.push(this.fileLine(o));return;}if(n==="readlines"&&a.length===0){if(o.closed)this.fail(i,"value","닫힌 파일은 사용할 수 없습니다.");if(o.mode!=="r")this.pythonFail(i,"UnsupportedOperation","쓰기 모드로 연 파일에서는 읽을 수 없습니다.");this.stack.push({kind:"list",items:this.fileLines(o)});return;}if(n==="write"&&a.length===1){this.stack.push(this.fileWrite(i,o,a[0]));return;}if(n==="close"&&a.length===0){this.close(o);this.stack.push(null);return;}this.fail(i,"name",`파일에서 '${n}' 메서드를 사용할 수 없습니다.`);}if(isList(o)){if(n==="append"&&a.length===1){o.items.push(a[0]);this.stack.push(null);return;}if(n==="pop"&&a.length<=1){if(!o.items.length)this.fail(i,"value","빈 리스트에서 pop할 수 없습니다.");const p=a.length?(typeof a[0]==="number"?a[0]:this.fail(i,"type","pop 인덱스는 정수여야 합니다.")):-1;this.stack.push(o.items.splice(p<0?o.items.length+p:p,1)[0]);return;}if(n==="remove"&&a.length===1){const x=o.items.findIndex(v=>elementEqual(v,a[0]));if(x<0)this.fail(i,"value","리스트에 해당 값이 없습니다.");o.items.splice(x,1);this.stack.push(null);return;}}if(isDict(o)){if(n==="keys"&&a.length===0){this.stack.push({kind:"list",items:o.entries.map(e=>e.key)});return;}if(n==="values"&&a.length===0){this.stack.push({kind:"list",items:o.entries.map(e=>e.value)});return;}if(n==="items"&&a.length===0){this.stack.push({kind:"list",items:o.entries.map(e=>({kind:"tuple",items:[e.key,e.value]}))});return;}if(n==="get"&&(a.length===1||a.length===2)){this.stack.push(o.entries.find(e=>elementEqual(e.key,a[0]))?.value??(a[1]??null));return;}}if(typeof o==="string"){if(n==="split"&&a.length<=1){const s=a[0];if(s!==undefined&&typeof s!=="string")this.fail(i,"type","split 구분자는 문자열이어야 합니다.");this.stack.push({kind:"list",items:s===undefined?o.trim().split(/\s+/).filter(Boolean):o.split(s)});return;}if(n==="strip"&&a.length===0){this.stack.push(o.trim());return;}if(n==="replace"&&a.length===2&&typeof a[0]==="string"&&typeof a[1]==="string"){this.stack.push(o.split(a[0]).join(a[1]));return;}}this.fail(i,"name",`'${n}' 메서드를 사용할 수 없습니다.`);}
  private binary(i: Extract<Instruction, { op: "binary" }>, left: Value, right: Value): Value {
    if (i.inplace && i.operator === "+" && isList(left)) {
      const values = this.items(i, right);
      if (left.items.length + values.length > MAX_COLLECTION_SIZE) this.systemFail(i, "컬렉션 크기 제한을 초과했습니다.");
      left.items.push(...values); return left;
    }
    ...
    let result: number;
    switch (i.operator) { case "+": result = a + b; break; case "-": result = a - b; break; case "*": result = a * b; break; case "/": result = a / b; break; case "//": result = isFloat(left) || isFloat(right) ? floatDivmod(a, b)[0] : Math.floor(a / b); break; case "%": result = isFloat(left) || isFloat(right) ? floatDivmod(a, b)[1] : a - Math.floor(a / b) * b; break; case "**": if (a === 0 && b < 0) this.fail(i, "value", "0으로 나눌 수 없습니다."); if (a < 0 && !Number.isInteger(b)) this.fail(i, "value", "복소수는 지원하지 않습니다."); result = a ** b; break; default: return this.fail(i, "runtime", "지원하지 않는 연산입니다."); }
    return isFloat(left) || isFloat(right) || i.operator === "/" || (i.operator === "**" && b < 0) ? float(result) : result;
  }
  private compare(i: Instruction, operator: string, left: Value, right: Value): boolean { if (operator === "is" || operator === "is not") return operator === "is" ? left === right : left !== right; if (operator === "==") return equal(left, right); if (operator === "!=") return !equal(left, right);
    if ((isList(left) && isList(right)) || (isTuple(left) && isTuple(right))) {
      for (let index = 0; index < Math.min(left.items.length, right.items.length); index++) {
        if (left.items[index] === right.items[index] || elementEqual(left.items[index], right.items[index])) continue;
        return this.compare(i, operator, left.items[index], right.items[index]);
      }
      return this.compare(i, operator, left.items.length, right.items.length);
    }
    if (isSet(left) && isSet(right)) {
      const subset = (a: Extract<Value, {kind: "set"}>, b: Extract<Value, {kind: "set"}>) => a.items.every(value => b.items.some(other => value === other || elementEqual(value, other)));
      if (operator === "<") return left.items.length < right.items.length && subset(left, right);
      if (operator === "<=") return subset(left, right);
      if (operator === ">") return left.items.length > right.items.length && subset(right, left);
      if (operator === ">=") return subset(right, left);
    }
  ...
  private invoke(i: Instruction, callable: Value, args: Value[], keywords: Record<string, Value>): void {
    ...
    if (name === "int" && args.length === 1 && isFloat(args[0]) && !Object.keys(keywords).length) { if (!Number.isFinite(args[0].value)) this.pythonFail(i, Number.isNaN(args[0].value) ? "ValueError" : "OverflowError", "무한대 또는 NaN은 정수로 변환할 수 없습니다."); this.stack.push(Math.trunc(args[0].value)); return; }
    if (name === "sum") {
      ...
      if (typeof total === "string") this.fail(i, "type", "sum()의 시작값으로 문자열은 사용할 수 없습니다."); for (const value of this.items(i, args[0])) { if (!numeric(total) || !numeric(value)) this.fail(i, "type", "sum()에는 숫자만 사용할 수 있습니다."); total = this.binary({ op: "binary", operator: "+", line: i.line, column: i.column }, total, value); } this.stack.push(total); return;
    }
    ...
    if (name === "map") { onlyNoKeywords(); if (args.length < 2) this.fail(i, "type", "map()은 함수와 반복 가능한 값이 필요합니다."); if (!isBuiltin(args[0]) && !isFunction(args[0]) && !isBoundMethod(args[0]) && !isClass(args[0]) && !isTurtleMethod(args[0]) && !isErrorType(args[0])) this.fail(i, "type", "map()의 첫 번째 인수는 호출할 수 있어야 합니다."); this.stack.push({ kind: "map", fn: args[0], sources: args.slice(1).map(value => this.iterator(i, value)) }); return; }
    ...
    if (name === "int") { if (typeof value === "number") { this.stack.push(Math.trunc(value)); return; } if (typeof value === "boolean") { this.stack.push(value ? 1 : 0); return; } if (typeof value !== "string") this.fail(i, "type", "정수로 변환할 수 없습니다."); const text = value.trim(); if (!/^[+-]?[0-9](?:_?[0-9])*$/.test(text)) this.fail(i, "value", "정수로 변환할 수 없습니다."); const number = Number(text.replaceAll("_", "")); if (!Number.isFinite(number) || !Number.isInteger(number)) this.fail(i, "value", "정수로 변환할 수 없습니다."); this.stack.push(number); return; }
...
import { floatDivmod, float, isFloat, numeric, numberValue } from "./numbers-v2";
